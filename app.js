...
function fresh(){
 ...
 symbionts:[],effects:[],niches:[],patches:[],carcasses:[],particles:[],ambient:[],restAnchor:null,weather:makeWeather(),camera:{lookX:0,lookY:0,zoom:1,eventX:null,eventY:null,eventTimer:0},ecosystem:{births:0,deaths:0,mergers:0,hunts:0},lineageActions:{hunts:0,engulfs:0,migrations:0,mergers:0,signals:0,inspections:0,niches:0,rests:0,scavenges:0,harshCycles:0},buildVersion:BUILD_VERSION,saveSchema:SAVE_SCHEMA,migrationReport:"New lineage · schema 6",resources:[],organisms:[],logs:[],lastInteraction:0
 };
}
let state=fresh();
...
function load(){
 ...
 for(const sourceKey of keys){
  ...
  try{
   ...
   state.particles=[];state.ambient=[];state.interactionTarget=null;state.buildVersion=BUILD_VERSION;state.saveSchema=SAVE_SCHEMA;state.migrationReport=sourceKey===SAVE_KEY?"Current save verified · schema 6":sourceKey===BACKUP_SAVE_KEY?"Recovered from rolling backup · schema 6":`Migrated from ${sourceKey} · schema 6`;state.logs=Array.isArray(x.logs)?x.logs.slice(0,120):[];
...
function createGameRuntime(engine){
 applyBuildIdentity();
...
window.__EVOLVA_BOOT_PHASE__="bootstrap";
(function(){
  function fail(error){
    console.error("EVOLVA startup failure:",error);
    window.__EVOLVA_BOOT_PHASE__="failed";
    var message=error&&error.message?error.message:String(error||"unknown error");
    var box=document.getElementById("error");
    if(box){box.hidden=false;box.textContent="Startup failed during "+(window.__EVOLVA_BOOT_STAGE__||"initialization")+": "+message;}
    window.dispatchEvent(new CustomEvent("evolva-runtime-failed",{detail:{message:message,stage:window.__EVOLVA_BOOT_STAGE__||"initialization"}}));
  }
  try{
    window.__EVOLVA_BOOT_STAGE__="engine construction";
    var engine=new EvolvaEngine({stepHz:60,maxCatchUpSteps:5});
    window.__EVOLVA_BOOT_STAGE__="game runtime creation";
    var game=createGameRuntime(engine);
    ...
    window.__EVOLVA_BOOT_STAGE__="engine start";
    engine.start();
    window.__EVOLVA_BOOT_PHASE__="ready";
    window.__EVOLVA_BOOT_STAGE__="ready";
